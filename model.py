...
def pct_change_to_prices(pct_changes, base_price):
    prices = np.zeros(len(pct_changes))
    current_price = base_price

    for i in range(len(pct_changes)):
        current_price = current_price * (1 + pct_changes[i])
    ...
    for epoch in range(epochs):
        ...
        with torch.no_grad():
            for x, y, base_price in test_loader:
                x, y, base_price = x.to(device), y.to(device), base_price.to(device)
                preds = model(x)
                ...
                for i in range(len(preds)):
                    pred_prices = torch.tensor(
                        pct_change_to_prices(preds[i].cpu().numpy(), base_price[i].cpu().item()),
                        device=device
                    )
                    actual_prices = torch.tensor(
                        pct_change_to_prices(y[i].cpu().numpy(), base_price[i].cpu().item()),
                        device=device
                    )
                    test_price_loss += criterion(pred_prices, actual_prices).item()

            test_loss /= len(test_loader.dataset)
            test_price_loss /= len(test_loader.dataset)

        epoch_time = time.time() - epoch_start_time

        train_pct_loss = np.sqrt(train_loss) * 100
        test_pct_loss = np.sqrt(test_loss) * 100
        test_dollar_loss = np.sqrt(test_price_loss)

        print(f"Epoch {epoch + 1}/{epochs} | Train % Loss: {train_pct_loss:.3f}% | "
              f"Test % Loss: {test_pct_loss:.3f}% | "
              f"Difference: {train_pct_loss - test_pct_loss:.3f}% || "
              f"Test $ Loss: ${test_dollar_loss:.2f} | "
...
def predict(model, sample, base_price, lookback=10):
    ...
    with torch.no_grad():
        ...
        # Convert percent changes to prices
        pred_prices = pct_change_to_prices(preds, base_price)
...
def predict_future(model, ticker: str, lookback=10, forecast_days=5, period='1y'):
    ...
    with torch.no_grad():
        ...
        pred_prices = pct_change_to_prices(preds, latest_base_price)
...
def get_loaders(tickers: list[str], training_proportion, period='1y', lookback=10, forecast_days=5):
    ...
    BP = np.concatenate(list(all_base_prices.values()), axis=0)
    ...
    training_samples = [X_shuffled[:num_training_samples], BP_shuffled[:num_training_samples],
                        D_shuffled[:num_training_samples], T_shuffled[:num_training_samples]]
    testing_samples = [X_shuffled[num_training_samples:], BP_shuffled[num_training_samples:],
                       D_shuffled[num_training_samples:], T_shuffled[num_training_samples:]]
...
if __name__ == "__main__":
    ...
    tickers = [
        'SPY', 'XLK', 'XLF', 'XLV', 'XLE', 'XLI', 'XLY', 'XLRE', 'XLU', 'XLC',
        'SOXX', 'QTUM', 'EFA', 'EEM', 'IWM', 'GLD', 'SLV',
    ]

    tech = ['AAPL', 'MSFT', 'AMZN', 'GOOGL', 'META', 'NVDA', 'TSLA', 'AVGO', 'ORCL', 'PLTR']
    quantum = ['IONQ', 'RGTI', 'QBTS', 'ARQQ', 'QUBT']

    BASE_MODEL_PATH = os.path.join(MODELS_DIR, 'base_model_pct.pt')
    ...
    future_nvda_preds = predict_future(trained_model, 'NVDA', lookback=lookback, forecast_days=forecast_days)
    visualize_future(future_nvda_preds, ticker='NVDA', lookback=lookback)
    ...
    tuned_future_nvda_preds = predict_future(tech_model, 'NVDA', lookback=lookback, forecast_days=forecast_days)
    visualize_future(tuned_future_nvda_preds, ticker='NVDA', lookback=lookback)
    ...
    for i in range(5):
        ...
        actuals_pct = sample[lookback:, 3]
        actuals = pct_change_to_prices(actuals_pct, base_price)

        historical_pct = sample[max(0, lookback - 20):lookback, 3]
        historical_base = base_price / (1 + sample[lookback - 1, 3])

        historical_prices = np.zeros(len(historical_pct))
        current_price = historical_base if lookback > 20 else base_price
        for j in range(len(historical_pct)):
            current_price = current_price * (1 + historical_pct[j])
            historical_prices[j] = current_price
